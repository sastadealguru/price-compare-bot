...
async def extract_product_info_async(html, site_name):
    ...
    prompt = f"""Extract top 2 matches from this {site_name} HTML snippet.
...
async def search_product(update, context):
    ...
    await update.message.reply_text("⚡ Fetching data from all electronics stores... Please wait.")
    ...
    if not valid:
        await update.message.reply_text("❌ Product matching your query not found or store blocked the request. Try again in a minute.")
        return

    msg = "🏆 **Best Deals Found!**\n\n"
